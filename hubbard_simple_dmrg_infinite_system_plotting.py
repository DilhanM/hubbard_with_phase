...
if __name__ == "__main__":
    ...
    for m in [10, 25, 50] :
        energy = infinite_system_algorithm(L=L, m=m, n=4)  # n is the number of states

        #print(energy)
        # plt.figure(m, figsize=(8, 4))
        plt.title("Infinite algorithm length of superblock : " + str(L))
        plt.xlabel("Energy level")
        plt.ylabel("Energy / L")
        ax = plt.gca()
        ax.get_yaxis().get_major_formatter().set_useOffset(False)
        # print(energy / (block.length * 2))
        plt.plot(range(len(energy)), energy / L, 'x', label="m = " + str(m))
