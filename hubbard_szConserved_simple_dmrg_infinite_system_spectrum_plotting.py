...
if __name__ == "__main__":
    ...
    for m in [10, 25, 50] :
        plt.figure(m)
        for Sz in [0,1,-1]:
            energy = infinite_system_algorithm(L=L, m=m, n = 4, target_Sz=Sz)

            plt.title("Infinite algorithm length of superblock : " + str(L) + "  m : "+ str(m))
            plt.xlabel("Energy level")
            plt.ylabel("Energy / L")
            ax = plt.gca()
            ax.get_yaxis().get_major_formatter().set_useOffset(False)
       # print(energy / (block.length * 2))
            plt.plot(range(len(energy)), energy / L, 'x', label="$S_z$ = " + str(Sz))
